...
describe('GoalFacade', () => {
  ...
  it('should be created', () => {
    const facade = TestBed.inject(GoalFacade);
  ...
  it('returns hasGoal as true when a goal exists', async () => {
    ...
    goalService.getGoal$.mockReturnValue(of({ targetAmount: 250_000, currency: 'USD' as const }));
  ...
  it('returns the goal target amount when present', async () => {
    mockUser.set({ uid: 'uid-1' });
    goalService.getGoal$.mockReturnValue(of({ targetAmount: 180_000, currency: 'USD' as const }));
  ...
  it('throws when saving while unauthenticated', () => {
    const facade = TestBed.inject(GoalFacade);
    expect(() => facade.save(150_000)).toThrow('Not authenticated');
  });
});
